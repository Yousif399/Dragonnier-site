...
function calculateTax(amount) {
  ...
  return (document.querySelector(".taxes").innerHTML = `$ ${
    Math.floor(pluTaxAmount * 100) / 100
  }`);
}
...
function calculateTotal(subtotal, taxes) {
  ...
  //   console.log(total);
  return (document.querySelector(".total-element").innerHTML = `$ ${
...
const buyItem = async (button) => {
  ...
  const url = `https://dragonnier-site.up.railway.app/place-order`;
  const options = {
    method: "POST",
    headers: {
...
const onSubmit = async (event) => {
  ...
  console.log("submitting is processing...");
  ...
  if (productFile) {
    data.append("productFile", productFile);
  }

  //   console.log("dataaa", ...data);

  const url = `https://dragonnier-site.up.railway.app/create-product`;
  const options = {
    method: "POST",
    body: data,
  };
  ...
  try {
    ...
    // console.log(response, "and  :", newData);
    if (response.status === 201 || response.status === 200) {
      console.log("data is uploaded", newData);
...
const fetchProducts = async () => {
  try {
    const response = await fetch(
      "https://dragonnier-site.up.railway.app/product"
    );
...
const deleteProduct = async (id) => {
  ...
  try {
    // console.log(id)
    loadingPopUP.classList.add("active");
    const url = `https://dragonnier-site.up.railway.app/delete-product/${id}`;
    const options = {
      method: "DELETE",
    };
    const response = await fetch(url, options);
...
const createProductCardInHome = (product) => {
  ...
  li.innerHTML = `
    <div class="${
      product.id === 1 ? "shop-card" : "shop-card grid"
    }"  id="first-grid">

    <figure class="card-banner img-holder" style="--width: 860; --height: 645;">
    <img src="${
      product.productImg
    }" width="860" height="646" loading="lazy" alt="${
    product.productName
  }" class="img-cover">
  </figure>
...
const getUpdatesInfo = async (data, id) => {
  ...
  const url = `https://dragonnier-site.up.railway.app/update-product/${id}`;
  const options = {
    method: "PATCH",
    headers: {
      "Content-Type": "application/json",
  ...
  if (response.status === 201 || response.status === 200) {
    ...
    const data = await response.json();
    alert("Couldn't post/update product check the server ", data);
  }
};
  ...
  try {
    ...
    products.forEach((product, index) => {
      ...
      if (
        ...
        window.location.pathname === "/post-product"
      ) {
